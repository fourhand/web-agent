...
class LangGraphSidepanel {
    ...
    setupEventListeners() {
        ...
        // 입력 필드에서 포커스가 벗어날 때도 자동 시작 (선택사항)
        document.getElementById('goalInput').addEventListener('blur', () => {
            const goal = document.getElementById('goalInput').value.trim();
            if (goal && this.status === "idle") {
                // 자동 시작을 원하지 않으면 이 부분을 주석 처리
                // this.startWorkflow();
            }
        });
    }
